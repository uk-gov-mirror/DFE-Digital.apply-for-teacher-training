...
RSpec.describe UCASMatch do
  ...
  describe '#action_needed?' do
    it 'returns false if ucas match has been manually resolved' do
      ucas_match = build_stubbed_dual_application_ucas_match(action_taken: 'manually_resolved')

      expect(ucas_match.action_needed?).to eq(false)
    end

    it 'returns false if ucas match is resolved' do
      ucas_match = build_stubbed_dual_application_ucas_match(action_taken: 'resolved_on_ucas')
    ...
    it 'returns false if initial emails were sent and we do not need to send the reminders yet' do
      ...
      ucas_match = build_stubbed_dual_application_ucas_match(action_taken: 'initial_emails_sent', candidate_last_contacted_at: initial_emails_sent_at)
    ...
    it 'returns true if initial emails were sent and it is time to send a reminder email' do
      ucas_match = build_stubbed_dual_application_ucas_match(:need_to_send_reminder_emails)
    ...
    it 'returns false if reminder emails were sent and we do not need to request withdrawal from UCAS yet' do
      ucas_match = build_stubbed_dual_application_ucas_match(action_taken: 'reminder_emails_sent', candidate_last_contacted_at: Time.zone.now)

      expect(ucas_match.action_needed?).to eq(false)
    end

    it 'returns true if reminder emails were sent and it is time to request withdrawal from UCAS' do
      ucas_match = build_stubbed_dual_application_ucas_match(:need_to_request_withdrawal_from_ucas)

      expect(ucas_match.action_needed?).to eq(true)
    end

    it 'returns false if we requested withdrawal from UCAS' do
      ucas_match = build_stubbed_dual_application_ucas_match(action_taken: 'ucas_withdrawal_requested', candidate_last_contacted_at: Time.zone.now)

      expect(ucas_match.action_needed?).to eq(false)
    end

    it 'returns true if there is a dual application or dual acceptance' do
      ucas_match = build_stubbed_dual_application_ucas_match
  ...
  describe '#ready_to_resolve?' do
    it 'returns true if no further action is required and the match is not resolved' do
      ucas_match = build_stubbed_dual_application_ucas_match(action_taken: 'ucas_withdrawal_requested')
  ...
  describe '#need_to_send_reminder_emails?' do
    it 'returns false if last action taken in not initial emails sent' do
      ...
      ucas_match = build_stubbed_dual_application_ucas_match(action_taken: 'ucas_withdrawal_requested', candidate_last_contacted_at: emails_sent_at)
    ...
    it 'returns false if initial emails were sent and we do not need to send the reminders yet' do
      emails_sent_at = Time.zone.now
      ucas_match = build_stubbed_dual_application_ucas_match(action_taken: 'initial_emails_sent', candidate_last_contacted_at: emails_sent_at)
  ...
  describe '#need_to_request_withdrawal_from_ucas?' do
    ...
    it 'returns false if reminder emails were sent and we do not need to request withdrawal from ucas yet' do
      emails_sent_at = Time.zone.now
      ucas_match = build_stubbed_dual_application_ucas_match(action_taken: 'reminder_emails_sent', candidate_last_contacted_at: emails_sent_at)
  ...
  def build_stubbed_dual_application_ucas_match(params = {})
    ucas_match = build_stubbed(:ucas_match, params)
