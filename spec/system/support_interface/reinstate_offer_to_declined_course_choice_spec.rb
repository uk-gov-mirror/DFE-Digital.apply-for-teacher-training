...
RSpec.feature 'Reinstate offer to a declined course choice' do
  ...
  scenario 'Support user can reverse a course choice that has accidently been declined' do
    ...
    and_i_provide_an_invalid_zendesk_ticket_link
  ...
  def then_i_am_told_to_confirm_i_have_followed_the_guidance
    expect(page).to have_content 'You must confirm you adhered to the guidance'
  end
  ...
  def then_i_am_told_that_i_need_to_provide_a_zendesk_ticket
    expect(page).to have_content 'Please provide a Zendesk ticket reference'
  end

  def and_i_provide_an_invalid_zendesk_ticket_link
    fill_in('Zendesk ticket', with: 'This wont work')
  end

  def then_i_am_told_that_i_need_to_provide_a_valid_zendesk_ticket_link
    expect(page).to have_content 'Please provide a valid Zendesk ticket link'
  end

  def when_i_provide_a_valid_zendesk_ticket
    fill_in('Zendesk ticket', with: 'https://becomingateacher.zendesk.com/agent/tickets/example')
  end
