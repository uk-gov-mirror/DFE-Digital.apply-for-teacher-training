  ...
  describe '.for' do
    context 'when the configurable provider notifications feature flag is on' do
      ...
      it 'returns training provider users for the application choice for a given type of event' do
        ...
        create(:provider_user_notification, :all_off, provider_user: create(:provider_user, send_notifications: false, providers: [application_choice.course.provider]))
        create(:provider_user_notification)

        provider_user = create(:provider_user, send_notifications: true, providers: [application_choice.course.provider])
        create(:provider_user_notification, provider_user: provider_user)

        expect(NotificationsList.for(application_choice, event: :application_received).to_a).to eql([provider_user])
      end
      ...
      it 'returns training and ratifying provider users for the application choice for a given type of event' do
        ...
        expect(NotificationsList.for(application_choice, include_ratifying_provider: true, event: :application_received).to_a).to match_array([training_provider_user, ratifying_provider_user])
      end
    end
