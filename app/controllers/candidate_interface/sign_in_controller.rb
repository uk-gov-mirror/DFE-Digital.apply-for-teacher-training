  ...
  class SignInController < CandidateInterfaceController
    ...
    def confirm_authentication
      ...
      if @authentication_token&.still_valid?
        emit_event('Sign in confirmation page visit', candidate: candidate)
        ...
        # a new sign in email.
        emit_event('Sign in link expired', candidate: candidate)
        redirect_to candidate_interface_expired_sign_in_path(token: params[:token], path: params[:path])
      elsif params[:u]
        emit_event('Legacy sign in link expired', candidate: candidate)
    ...
    def authenticate
      ...
      if authentication_token && authentication_token.still_valid?
        ...
        emit_event('Sign in success', candidate: candidate)
