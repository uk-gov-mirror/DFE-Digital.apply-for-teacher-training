module SupportInterface
  class ApplicationChoiceComponent < ViewComponent::Base
    ...
    def rows
      rows = if FeatureFlag.active?(:support_user_reinstate_offer) && application_choice.declined?
               [
                 { key: 'Status', value: render(SupportInterface::ApplicationStatusTagComponent.new(status: application_choice.status)) +
                   govuk_link_to('Reinstate offer', support_interface_application_form_reinstate_offer_path(application_form_id: @application_choice.application_form.id, application_choice_id: @application_choice.id)) },
               ]
             else
               [{ key: 'Status', value: render(SupportInterface::ApplicationStatusTagComponent.new(status: application_choice.status)) }]
             end
